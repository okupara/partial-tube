...
import {ApolloServer, gql, AuthenticationError} from "apollo-server-cloud-functions"
...
const veriyfyToken = (token: string | undefined) => {
  ...
  return admin.auth().verifyIdToken(token).catch(e => new InvalidTokenError())
}
...
const apolloServer = new ApolloServer({
  ...
  resolvers: {
    Query: {
      verify: (a, b, c, d) => (console.log(a,b,c,d), {
        name: "oooooooooo"
      })
    }
  },
